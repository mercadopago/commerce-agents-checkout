...
the body, required attempt-scoped identifier pair, idempotency header,
price/currency/stock confirmation, bounded quantity conversion, validation of the returned
Order snapshot, URL allowlist, failure paths, indeterminate outcomes, cancellation cleanup,
and log redaction.
...
That mode repeats the create call with the same idempotency key and external reference,
confirms that Mercado Pago returns the original Order, and verifies the reference on
read-back. To print the Order ID and URL for a browser check, run from an interactive
terminal with `MERCADOPAGO_LIVE_SHOW_CHECKOUT_URL=1`; redirected or captured output is
rejected.
...
1. The script retains exactly one Order ID internally and withholds it unless sensitive
   output is explicitly enabled in an interactive terminal.
...
6. The Order read-back contains the seller reference generated for the attempt.
7. Replaying the same key and reference returns the same Order ID and URL.

For `verify-cancellation`, the script retains the created Order ID internally and proves a
GET observed `status=canceled`; it withholds the Order ID and checkout URL by default.

The script does **not** prove durable host behavior; verify this in a host that persists
state:

- atomically persisting the idempotency key, external reference, and Order snapshot before
  the first call, then mapping each webhook to exactly one attempt by that reference.

During integration validation, an opt-in live Checkout Pro Orders request without
`external_reference` returned HTTP 400 with `required_properties` and identified
`external_reference` as missing. The published Orders API reference and supported Python
SDK currently describe the field as optional, so this package deliberately adopts a
stricter required-field contract for the tested Checkout Pro path and reconciliation. The
observation does not establish that every Orders product or account rejects omission.
...
The order expires after `P1D`. Do not paste the token, Order ID, recovery identifiers, or
complete checkout URL into public issues or CI logs.
