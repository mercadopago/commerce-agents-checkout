...
- Documents `recovering: bool = False` as part of the public surface in
  ...
  documented signature against `inspect.signature`, so the formal contract cannot drift
  away from the code again.
- Publishes the wrapper that actually carries the recovery state: the README and
  module-docstring snippets forward `recovering=attempt.outcome_unknown`, and the
  guidance says plainly that persisting the two identifiers without persisting *that
  the outcome was unknown* leaves the gap open. `examples/seller_integration.py` takes
  the stricter route and refuses to call the adapter again until the attempt is
  reconciled; both routes are covered by tests.
- Refuses to release the host fallback while recovering. `checkout_handoff` takes an
  optional `recovering=True`, which the host passes when it retries after a
  `CheckoutOutcomeUnknown`. In that mode a refusal decided locally — a catalog price
  that moved between the two calls, for instance — raises instead of returning `[]`,
  because an earlier attempt under the same key may already have created a payable
  Order that this call never asked Mercado Pago about. The default is off and the
  upstream two-argument call is unchanged.
- Validates the idempotency key against what the HTTP stack will actually transmit
