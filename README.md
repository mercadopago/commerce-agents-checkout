...
class MyBackend(StorefrontBackend):
    ...
    async def checkout_handoff(self, session, cart):
        ...
        return await self.mercadopago.checkout_handoff(
            ...
            # Set once this attempt has already ended in CheckoutOutcomeUnknown.
...
reuse that same pair for every retry of the confirmed purchase. A retry that follows a
`CheckoutOutcomeUnknown` needs one thing more: either pass `recovering=True`, so that a
refusal decided locally raises instead of releasing your fallback, or refuse to call the
adapter at all until the attempt is reconciled, which is what
...
does. Persisting the two identifiers without also persisting *that the outcome was
unknown* leaves the original gap open. What you get from the package:
...
Install the stable release and its dependencies from PyPI:

```bash
python -m pip install mercadopago-commerce-agents-checkout==0.1.0
```

The explicit pin makes deployments reproducible. Upgrade deliberately after reviewing
the changelog; pre-release versions are not selected unless the caller requests one.
...
The adapter surface is two constructor arguments, two required keyword-only
identifiers, and one optional recovery flag per call:
...
`idempotency_key` identifies one checkout operation, and `recovering` says whether this
call is a retry of one that already ended in `CheckoutOutcomeUnknown`:
...
handoffs = await checkout.checkout_handoff(
    ...
    external_reference=attempt.external_reference,
    idempotency_key=attempt.idempotency_key,
    recovering=attempt.outcome_unknown,  # bool, defaults to False
)
```
...
- `recovering` defaults to `False`. Persist the inconclusive outcome together with the
  two identifiers and forward it on every retry that follows a `CheckoutOutcomeUnknown`:
  it is what keeps a locally decided refusal from releasing your fallback while an Order
  from the earlier attempt may still be payable.
...
| `CheckoutOutcomeUnknown` while `recovering=True` | A retry after an earlier indeterminate outcome was refused locally, so nothing in this call proved whether the first attempt created an Order. Reconcile by the `external_reference` before charging anywhere else. |
| `CheckoutOutcomeUnknown` with reason `resource_locked` (HTTP 423) | A request for this idempotency key is still in flight at Mercado Pago and may already have created an Order. Do not fall back. Repeat the same key after a short delay, then reconcile if it keeps failing. |
| `CheckoutOutcomeUnknown` with reason `uncorrelated_response` | The created-order response did not carry the `external_reference` that was sent, so the adapter could not prove the Order belongs to this attempt and deliberately did not cancel it. Reconcile using the exception's `order_id` before retrying. |
...
| `Order creation failed (HTTP 400)` | The request reached the account but failed schema validation. The logged `codes` combine recognized Orders API `errors[].code` values with legacy `error` and numeric `cause[].code` values. Descriptions and request data are never logged. |
