...
- `recovering` marks a retry that follows a `CheckoutOutcomeUnknown`. It is the host's
  job to persist that the attempt ended inconclusively, alongside the two identifiers,
  and to forward that state here. With it, a refusal decided locally raises instead of
  returning `[]`, because an Order created by the earlier attempt may still be payable
  and nothing in this call asked Mercado Pago about it. Hosts that would rather block
  than retry can refuse to call the adapter until the attempt is reconciled, which is
  what `examples/seller_integration.py` does; persisting the identifiers without the
  inconclusive state is the one combination that is not safe.
...
own checkout to take over — except while recovering. Pass `recovering=True` when
retrying after a `CheckoutOutcomeUnknown`: an Order carrying that same key may already
be payable, and a refusal decided locally never asked Mercado Pago about it, so in that
mode every refusal raises rather than releasing the fallback. Persisting the two
identifiers without persisting the inconclusive outcome is what leaves the gap open, so
the attempt record needs all three. The one exception is a
refused Order whose cancellation Mercado Pago confirmed in this same call, which does
leave `[]` safe. The 20-line and quantity-10 limits are package safety budgets,
